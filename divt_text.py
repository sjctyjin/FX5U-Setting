...
def WritePLC(i,PLC,k):
    ...
    SLMP = [
        ...
        0x00, 0x00,  # Request data length (7,8)
        0x00, 0x00,  # Monitoring timer (9,10)
        0x01, 0x14,  # Command (11,12)
        0x00, 0x00,  # Subcommand (13,14)
        #0x01,  # Word access points

    ]
    if k == 0:
        SLMP[15:] = 0xFD, 0x01, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R509
    elif k == 1:
        SLMP[15:] = 0x07, 0x02, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R519
    elif k == 2:
        SLMP[15:] = 0x11, 0x02, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R529
    elif k == 3:
        SLMP[15:] = 0x1B, 0x02, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R539
    elif k == 4:
        SLMP[15:] = 0x25, 0x02, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R549
    elif k == 5:
        SLMP[15:] = 0x2F, 0x02, 0x00, 0xAF,0x01,0x00,0x00,0x00, #R559

        # 0x00, 0xF1, 0x01, 0x00, 0xAF,  # R509
        # 0x00, 0x02, 0x02, 0x00, 0xAF,  # R519
        # 0x00, 0x0C, 0x02, 0x00, 0xAF,  # R529
        # 0x00, 0x16, 0x02, 0x00, 0xAF,  # R539
        # 0x00, 0x20, 0x02, 0x00, 0xAF,  # R549
        # 0x00, 0x2A, 0x02, 0x00, 0xAF,  # R559
    SLMP[7:9] = struct.pack('H', len(SLMP[9:]))  # Request data length
    try:
        print(SLMP)
        sock.connect(tuple(PLC[i][0:2]))
